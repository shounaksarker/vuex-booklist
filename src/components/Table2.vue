...
export default {
    ...
    computed: {
        ...
        filteredBooks() {
            // console.log("hello env : ",import.meta.env.VITE_API_KEY)
    ...
    mounted() {
        return {
            async fetchData() {
                await fetch(`https://www.googleapis.com/books/v1/volumes?q="harry potter"&key=${import.meta.env.VITE_API_KEY}`)
                    .then(resp => resp.json())
